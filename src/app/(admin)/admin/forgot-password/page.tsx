...
export default function ForgotPasswordPage() {
  ...
  async function handleSubmit(e: React.SyntheticEvent<HTMLFormElement>) {
    ...
    const { error: authError } = await supabase.auth.resetPasswordForEmail(email, {
      redirectTo: `${process.env.NEXT_PUBLIC_SITE_URL || window.location.origin}/auth/admin-callback?type=recovery`,
    })
      ...
      <div className="relative z-10 flex-1 flex flex-col items-center justify-center px-4">
        {sent ? (
          <div className="text-center">
            ...
            <p style={{ color: 'rgba(255,255,255,0.5)' }}>Password reset link sent to <span className="text-white font-semibold">{email}</span></p>
            <a href="/admin/login" className="block mt-6 text-sm" style={{ color: GREEN }}>← Back to login</a>
          </div>
        ) : (
          <div className="w-full max-w-sm">
            <div className="text-center mb-8">
              ...
              <p className="mt-1 text-sm" style={{ color: 'rgba(255,255,255,0.4)' }}>We'll send you a reset link</p>
            </div>
            ...
            <a href="/admin/login" className="block text-center text-xs mt-6" style={{ color: 'rgba(255,255,255,0.3)' }}>← Back to login</a>
          </div>
        )}
      </div>
    </div>
